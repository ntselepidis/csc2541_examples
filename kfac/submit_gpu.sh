    ...
    for random_seed in 0 1 2 3 4; do
        ...
        for optimizer in "kfac"; do
            for use_momentum in 1 0; do
                bsub -G ${group} -W 02:00 -n 4 -R "rusage[mem=8192,ngpus_excl_p=1]" -R "select[gpu_model0==NVIDIAGeForceGTX1080Ti]" \
        ...
        for optimizer in "conjgrad" "kfac-conjgrad"; do
            for conjgrad_maxiter in 5 10 20 40; do
                bsub -G ${group} -W 04:00 -n 4 -R "rusage[mem=8192,ngpus_excl_p=1]" -R "select[gpu_model0==NVIDIAGeForceGTX1080Ti]" \
        ...
        for nbasis in 1 5 10 20; do
            ...
            # two-level kfac runs
            for optimizer in "kfac-cgc" "kfac-cgc-m3"; do
                for use_momentum in 1 0; do
                    bsub -G ${group} -W 02:00 -n 4 -R "rusage[mem=8192,ngpus_excl_p=1]" -R "select[gpu_model0==NVIDIAGeForceGTX1080Ti]" \
            ...
            for optimizer in "kfac-cgc-conjgrad" "kfac-cgc-m3-conjgrad"; do
                for conjgrad_maxiter in 5 10 20 40; do
                    bsub -G ${group} -W 04:00 -n 4 -R "rusage[mem=8192,ngpus_excl_p=1]" -R "select[gpu_model0==NVIDIAGeForceGTX1080Ti]" \
